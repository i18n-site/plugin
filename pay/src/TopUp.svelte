...
+ card_li, stripe_li, card, rate, max, form, ed, n, STATUS
...
conv = (x)=>
  Math.ceil((x * rate)*100)

:$
  payN = (conv(n)/100).toFixed(2)
          ...
          +if ed
            p
              i
                | >topup
                |
                | { (Math.floor(ed[0] / rate) / 100).toFixed(2) }
                | >USD
            +else
              +if h
                h2 { h }
